...
- 検証済みの API 面: サーバー側 `/v2/`(純粋 API クライアントの静的ページ)で動くこと = アプリで実現できること
- API の正確な入出力は **API 仕様書 [KaraokeRequestorWeb の api/README.md](https://github.com/bee7813993/KaraokeRequestorWeb/blob/master/api/README.md)** を参照
- 本書はアプリ側の設計と、サーバー側(KaraokeRequestorWeb リポジトリ)に追加が必要な作業の両方を扱う

> 本書はサーバーリポジトリの `mobile_app_design.md` から移動してきた設計書で、以後はこのリポジトリで更新する。
...
| キャラクター | デフォルトはマスコットの「ゆかり」ちゃん(素材原本: `art/mascot/`)。Live2D・動画・画像でユーザーカスタマイズ可能 |
...
アプリのマイページ (履歴 / あとで歌う / お気に入り) は**端末ローカル保存** (mypage.json)。
スキンと同じ「端末内で完結」の方針で、サーバーには何も置かない (オフライン閲覧可・サーバー設定に非依存)。
履歴は予約成功時に自動記録し、あとで歌う / お気に入りは予約確認モーダルのトグルで登録する。

将来 Web 版と連携する場合の実装パス (バックログ):

1. **デバイスリンク**: 既存のペアリングコード (mypage_pair_code、5分TTL) をアプリから叩いて
   YkariUserID を Web と統一する。アプリは全リクエストに YkariUserID Cookie を送信済みのため、
   統一後はサーバー側にも同じユーザーとして履歴が蓄積される。
   サーバーの一覧取得 API は KaraokeRequestorWeb PR #207 (クローズ済み) を復活すればよい
2. **Google 同期**: mypage_google_sync の仕組みにアプリも乗せ、Google Drive 経由で Web 版と自動同期する

## 6. 背景・キャラクターのカスタマイズ設計

> 実装済みの全キー・挙動・フォールバック順のリファレンスは
> [skin-spec.md](skin-spec.md)。この章は設計意図と将来構想を含むラフスケッチ。
...
- キャラ: `art/mascot/yukari_mascot_transparent.png`(透過版)を使用
